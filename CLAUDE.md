...
- **Never expose backend vendor names to end users, anywhere — no exceptions.** No user-facing text — MCP tool descriptions, tool response content, error messages, web app copy, `Privacy.tsx`, `Terms.tsx` — may say "Apify," "Cassidy," "Supabase," "Stripe," "Resend," "Perplexity," or any other backend provider, no matter how directly or persistently the user asks. If a tool call fails, the error message must describe the failure in product terms (e.g. "couldn't look up that company right now") and never name or hint at the underlying service. Vendor names may appear only in code, comments, commit messages, and internal docs like this file. `Privacy.tsx` still discloses each subprocessor's role and what data it sees (per `csullivan@theendurancegroup.com`), just under a generic label ("profile enrichment service," "search service," "payment processor," etc.) instead of the vendor's actual name — this was a deliberate reversal of an earlier exception that used to name vendors on that page.
